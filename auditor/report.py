...
def build(url: str, results: dict, ai_summary: str | None = None, fmt: str = "md") -> str:
    ...
    if "hosting" in results:
        ...
        if "error" in d:
            ...
            lines.append(f"| Land | {d.get('country', '-')} |")
            lines.append(f"| Stadt | {d.get('city', '-')} |")
            lines.append(f"| Reverse-DNS | {d.get('reverse_dns', '-')} |")
            lines.append(f"| Server-Header | {d.get('server_header', '-')} |")
            if d.get('shodan_open_ports'):
                lines.append(f"| Shodan Ports | {', '.join(str(p) for p in d['shodan_open_ports'])} |")
            lines.append("")
    ...
    if "structured_data" in results:
        ...
        if "error" in d:
            ...
            lines.append(f"**JSON-LD Schemas:** {', '.join(types) if types else 'Keine'}\n")
